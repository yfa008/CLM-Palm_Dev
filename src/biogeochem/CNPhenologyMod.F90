...
!         p = filter_soilp(fp)
  ...
  subroutine PalmPhenology(num_pcropp, filter_pcropp                     , &
    ...
    ! !DESCRIPTION:
    ! !Phytomer based oil palm phenology subroutine developed by Yuanchao Fan 
    ! !reference: Fan, Y. et al. A sub-canopy structure for simulating oil palm in the Community Land Model (CLM-Palm): phenology, allocation and yield. Geoscientific Model Development 8, 3785–3800 (2015).
   ...
   do fp = 1, num_pcropp
      ...
      if ( phytomer(ivt(p)) > 0) then
         ...
         if (croplive(p)) then
             ...
             ! new counter for perennial crops (added by Y.Fan)
  ...
  subroutine CropPhenology(num_pcropp, filter_pcropp                     , &
    ...
    associate(                                                                   & 
         ...
         lfemerg           =>    pftcon%lfemerg                                , & ! Input:  
         grnfill           =>    pftcon%grnfill                                , & ! Input:
         grnfill2          =>    pftcon%grnfill2                               , & ! Input: 
      ...
      do fp = 1, num_pcropp
         ...
         if ( (.not. croplive(p)) .and. (.not. cropplant(p)) ) then
            ...
            if (ivt(p) == ntmp_corn .or. ivt(p) == nirrig_tmp_corn .or. &
               ...
               !initialize huigrain2 for perennial crops, 
               !then update it annually or monthly in the harvest cycle (Y.Fan)
         ...
         !onset_flag(p)  = 0._r8 ! CN terminology to trigger certain
         !offset_flag(p) = 0._r8 ! carbon and nitrogen transfers
         harvest_flag(p) = 0._r8 ! annual harvest flag for perennial crops (added by Y.Fan)
         ...
         if (croplive(p)) then
            ...
            ! Generic perennial crop phenology to enable continuous harvests over years (added by Yuanchao Fan)
            ! A perennial flag and a harvest flag are added (Y.Fan 06.06.2014)
            else if (hui(p) >= gddmaturity(p) .and. idpp(p) < mxmat(ivt(p)) .and. &
                     perennial(ivt(p)) == 1) then

               !huigrain = grnfill * gddmaturity ~=2.3 years, which is the time needed from planting to first fruitfill 
               !(1 - grnfill) * gddmaturity indicates the time needed from each fruitfill (huigrain2) to fruit maturity (gddmaturity2),
               !which is fixed at ~140 days needed for oil palm fruit bunches to ripe.
               gddmaturity2(p) = huigrain2(p) + (1._r8 - grnfill(ivt(p))) * gddmaturity(p) !GDD till next harvest

               !update huigrain2 after each harvest to initiate the next fruitfill of a younger phytomer
               !to enable fruit filling and harvest at monthly or annual time step (Y.Fan)
               if (hui(p) >= gddmaturity2(p)) then

                  !the following ensures a one-time harvest per month or per year for perennial crops
                  !set grnfill2 in the parameter file so that grnfill2 * gddmaturity ~= 1 month (300 GDD) for oil palm
                  !or 1 year (3600 GDD) for other perennial crops
                  !currently grnfill=0.86 (for initial fruitfill) and grnfill2=0.03 (for subsequent fruitfills) 
                  huigrain2(p) = huigrain2(p) + grnfill2(ivt(p)) * gddmaturity(p) !GDD when next fruitfill starts

                  !apply fertilizer after each harvest when new fruitfill begins (Y.Fan)
            ...
            ! Ashehad Ali added below codes for stress_deciduous crop phenology here
            ...
            if (semi_decid(ivt(p)) == 1._r8) then
               ...
               if (dormant_flag(p) == 1._r8) then
                 ...
                 ! similar to Ali et al. Implementing Rubber PFT in CLM5 Land (2022) 11(2):183  
                  ...
                  !generic perennial crop phenology to enable continuous harvests over years (added by Y.Fan) 
                  else if (hui(p) >= gddmaturity(p) .and. idpp(p) < mxmat(ivt(p)) .and. &
                      perennial(ivt(p)) == 1) then

                      gddmaturity2(p) = huigrain2(p) + (1._r8 - grnfill(ivt(p))) * gddmaturity(p) !GDD till next harvest
                      !gddmaturity2(p) = huigrain2(p) + (1._r8 - grnfill(ivt(p))) * gddmaturity(p)/12._r8 !

                      !update huigrain2 after each harvest to initiate the next fruitfill
                      !to enable fruit filling and harvest at monthly or annual time step (Y.Fan)
                      if (hui(p) >= gddmaturity2(p)) then

                         !the following ensures a one-time harvest per month or per year for perennial crops
                         !set grnfill2 in the parameter file so that grnfill2 * gddmaturity ~= 1 month (300 GDD) for monthly harvest
                         !or 1 year (3600 GDD) for annual harvests
                         huigrain2(p) = huigrain2(p) + grnfill2(ivt(p)) * gddmaturity(p) !GDD when next fruitfill starts
                         !huigrain2(p) = huigrain2(p) + 0.5_r8 * grnfill(ivt(p)) * gddmaturity(p)/12._r8 !
                   ...
                   ! similar to Ali et al. Implementing Rubber PFT in CLM5. Land (2022) 11(2):183
                      ...
                      if (hui(p) >= gddmaturity2(p)) then
                         !the following ensures a one-time harvest per month or
                         !per year for perennial crops
                         !set grnfill2 in the parameter file so that grnfill2 *
                         !gddmaturity ~= 1 month (300 GDD) for monthly harvest
                         !or 1 year (3600 GDD) for annual harvests
                         huigrain2(p) = huigrain2(p) + grnfill2(ivt(p)) * gddmaturity(p) !GDD when next fruitfill starts
                         !huigrain2(p) = huigrain2(p) + 0.5_r8 * grnfill(ivt(p)) * gddmaturity(p)/12._r8 !
  ...
  subroutine CNOffsetLitterfall (num_soilp, filter_soilp, &
    ...
    associate(                                                                           & 
      ...
      do fp = 1,num_soilp
         ...
         !incorporate a one-time harvest: annually for perennial crops; seasonally for phytomer phenology (added by Y.Fan)
         !leaf flux is treated according to leaf property (evergreen or deciduous) and management (prune or not)
         !grain flux goes to food c pool
  ...
  subroutine CNBackgroundLitterfall (num_soilp, filter_soilp, &
    ...
    associate(                                                                     & 
      ...
      do fp = 1,num_soilp
        ...
        ! and pruning of old phytomer (added by Y.Fan)
    ...
    associate(                                                                                   & 
      ...
      do fp = 1,num_soilp
	    ...
	    ntovr = livecrootn(p) * lwtop
          ...
          !considered 100% active (added by Y.Fan)
          if (phytomer(ivt(p)) > 0) then
